...
Функция ПолучитьФорматированноеСообщение(Знач СобытиеЛога) Экспорт
	...
	ОбработатьДопПоля(ДопПоля, СобытиеЛога.ПолучитьУровеньЛога());
...
Процедура ПриСозданииОбъекта(Знач ПВыводитьМеткиЦвета = Ложь, 
	...
	ПВыводитьМеткиЦвета = ?(ТипЗнч(ПВыводитьМеткиЦвета) = Тип("Булево"), ПВыводитьМеткиЦвета, Истина);
	ПВыводитьДату = ?(ТипЗнч(ПВыводитьДату) = Тип("Булево"), ПВыводитьДату, Истина);
	ПИспользоватьПрефиксы = ?(ТипЗнч(ПИспользоватьПрефиксы) = Тип("Булево"), ПИспользоватьПрефиксы, Истина);
